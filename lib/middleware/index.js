...
function initMiddleware (options) {
  ...
  const watchs = [...new Set([...options.watchs, options.confFile])]
  const watchFiles = []
  watchs.forEach(filepath => {
  ...
  watchFiles.length > 0 && utils.watch([...new Set(watchFiles)], (file) => {
    log.warning(`${file} changed!`)
    delete require.cache[file]
    // 如果是配置文件，则重新处理配置文件
    if (!file.includes(options.dataFilePath)) {
      handleConfig(options)
    }
  })
